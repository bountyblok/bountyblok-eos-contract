...
class [[eosio::contract("bountyblok")]] bountyblok : public contract {
  public:
	 ...
	 */
	[[eosio::action]]
	void execchlgdone(name receiver, name account_name, std::string challenge_name, std::string challenge_guid, uint64_t completion_date, asset reward) {
		notify_challenge_done_action notichlgdone(get_self(), {get_self(), "active"_n});
		notichlgdone.send(receiver, account_name, challenge_name, challenge_guid, completion_date, reward);
	}

	[[eosio::action]]
	void notichlgdone(name receiver, name account_name, std::string challenge_name, std::string challenge_guid, uint64_t completion_date, asset reward) {
